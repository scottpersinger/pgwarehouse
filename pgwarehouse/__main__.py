...
parser.add_argument(
    'command', 
    help="Command",
    choices=['list','listwh','extract','load','sync','resync','update']
)
...
pgw = PGWarehouse(config_file=args.config, command=args.command, table=args.table,
                  data_dir=args.data, backend=args.backend)
